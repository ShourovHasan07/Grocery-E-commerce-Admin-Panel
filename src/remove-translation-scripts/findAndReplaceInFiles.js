...
const getDictionaryProp =
  /const\s+dictionary\s+=\s+await\s+getDictionary\(params\.lang\)\s*/g;

const replaceDirectionPattern =
  /const direction\s*=\s*i18n.langDirection\[params.lang\]/g;

const removeDictionaryDestructuringPattern = /dictionary(,| )/g;

// Pattern to match Props type definitions including `params`

const removeParamsFromFunctionPattern =
  /(?<={ .*)params,?(?=.* }: [A-Z][A-Za-z]+)/g;
