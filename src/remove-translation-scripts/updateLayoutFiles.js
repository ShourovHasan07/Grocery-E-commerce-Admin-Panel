...
export const updateLayoutFile = async () => {
  ...
  let layoutFileContent = await fs.promises.readFile(
    "src/app/layout.jsx",
    "utf8",
  );
  ...
  let notFoundFileContent = await fs.promises.readFile(
    "src/app/[...not-found]/page.jsx",
    "utf8",
  );

  notFoundFileContent = notFoundFileContent.replace(
    /const params = await props.params/g,
    "",
  );
...
export const updateDashboardLayoutFile = async () => {
  ...
  content = content
    .replace(
      /<Customizer((?!disableDirection)[^>]*?)\/?>/g,
      `<Customizer$1 disableDirection />`,
    )
...
export const updateFrontLayoutFile = async () => {
  ...
  content = content.replace(
    /<((html|body).*?)>\s*<InitColorSchemeScript.*?\/>(.*?)<\/body>\s*<\/html>/gs,
    "$3",
  );
  await fs.promises.writeFile(filePath, content);
