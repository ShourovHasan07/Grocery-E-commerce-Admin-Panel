...
export const updateAuthGuard = async () => {
  ...
  let AuthGuardFileContent = await fs.promises.readFile(
    "src/hocs/AuthGuard.jsx",
    "utf8",
  );

  // Modify the file content as needed
  AuthGuardFileContent = AuthGuardFileContent.replace(
...
export const updateGuestOnlyRoutes = async () => {
  ...
  // Using fs.promises API for reading and writing files asynchronously
  let GuestOnlyRouteFileContent = await fs.promises.readFile(
    "src/hocs/GuestOnlyRoute.jsx",
    "utf8",
  );

  // Modify the file content as needed
  GuestOnlyRouteFileContent = GuestOnlyRouteFileContent.replace(
