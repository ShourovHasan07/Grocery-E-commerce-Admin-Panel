...
const ChatLog = ({
  ...
  const activeUserChat = chatStore.chats.find(
    (chat) => chat.userId === chatStore.activeUser?.id,
  );
  ...
  return (
    <ScrollWrapper isBelowLgScreen={isBelowLgScreen} scrollRef={scrollRef}>
      <CardContent className="p-0">
        {activeUserChat &&
          formatedChatData(activeUserChat.chat, profileUser).map(
            (msgGroup, index) => {
              ...
              return (
                <div
                  key={index}
                  className={classnames("flex gap-4 p-6", {
                    "flex-row-reverse": isSender,
                  })}
                >
                  {!isSender ? (
                    contacts.find(
                      (contact) => contact.id === activeUserChat?.userId,
                    )?.avatar ? (
                      <Avatar
                        alt={
                          contacts.find(
                  ...
                  >
                    {msgGroup.messages.map((msg, index) => (
                      <Typography
                        key={index}
                        className={classnames(
                          "whitespace-pre-wrap pli-4 plb-2 shadow-xs",
                          {
