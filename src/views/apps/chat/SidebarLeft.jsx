...
const renderChat = (props) => {
  ...
  return chatStore.chats.map((chat) => {
    const contact =
      chatStore.contacts.find((contact) => contact.id === chat.userId) ||
      chatStore.contacts[0];
    ...
    return (
      <li
        ...
        className={classnames(
          "flex items-start gap-4 pli-3 plb-2 cursor-pointer rounded mbe-1",
          {
      ...
      >
        ...
        <div className="flex flex-col items-end justify-start">
          ...
          >
            {chat.chat.length
              ? formatDateToMonthShort(chat.chat[chat.chat.length - 1].time)
              : null}
          </Typography>
          {chat.unseenMsgs > 0 ? (
            <CustomChip
...
const SidebarLeft = (props) => {
  ...
  const handleChange = (event, newValue) => {
    ...
    dispatch(
      addNewChat({
        id: chatStore.contacts.find((contact) => contact.fullName === newValue)
          ?.id,
      }),
    );
    getActiveUserData(
      chatStore.contacts.find((contact) => contact.fullName === newValue)?.id ||
        ...
        <div className="flex items-center plb-[18px] pli-6 gap-4 border-be">
          ...
          <div className="flex is-full items-center flex-auto sm:gap-x-3">
            <Autocomplete
              ...
              id="select-contact"
              options={
                chatStore.contacts.map((contact) => contact.fullName) || []
              }
              ...
              renderOption={(props, option) => {
                const contact = chatStore.contacts.find(
                  (contact) => contact.fullName === option,
                );
