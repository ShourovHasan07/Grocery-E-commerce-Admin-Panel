...
const ScrollWrapper = ({ children, isBelowLgScreen }) => {
  if (isBelowLgScreen) {
    return (
      <div className="bs-full overflow-y-auto overflow-x-hidden relative">
        {children}
      </div>
    );
  } else {
...
const MailContentList = (props) => {
  ...
  const handleEmailClick = (id) => {
    ...
    if (
      store.currentEmailId !== id ||
      emails.find((email) => email.id === id)?.isRead === false
    ) {
  ...
  return isInitialMount ? (
    ...
    <div className="relative flex justify-center gap-2 grow is-full bg-backgroundPaper">
      ...
      {reload && (
        <Backdrop
          open={reload}
          className="absolute text-white z-10 bg-textDisabled"
        >
              ...
              >
                <div className="flex items-center justify-between gap-2">
                  <div className="flex items-center gap-2 overflow-hidden">
                    ...
                    />
                    <IconButton
                      onClick={(e) => handleToggleStarEmail(e, email.id)}
                    >
                      <i
                  ...
                  {!isBelowLgScreen && (
                    <div className={styles.emailActions}>
                      ...
                      >
                        <IconButton
                          onClick={(e) => handleSingleEmailDelete(e, email.id)}
                        >
                      ...
                      {(folder === "inbox" || folder === "trash") && (
                        <Tooltip title="Move to spam" placement="top">
                          <IconButton
                            onClick={(e) => handleMoveToSpam(e, email.id)}
                          >
