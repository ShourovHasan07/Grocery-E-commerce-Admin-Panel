      ...
      >
        <i
          className={classnames("tabler-bold", {
            "text-textPrimary": !editor.isActive("bold"),
          })}
        />
      </CustomIconButton>
      <CustomIconButton
      ...
      >
        <i
          className={classnames("tabler-underline", {
            "text-textPrimary": !editor.isActive("underline"),
          })}
        />
      </CustomIconButton>
      <CustomIconButton
      ...
      >
        <i
          className={classnames("tabler-italic", {
            "text-textPrimary": !editor.isActive("italic"),
          })}
        />
      </CustomIconButton>
      <CustomIconButton
      ...
      >
        <i
          className={classnames("tabler-strikethrough", {
            "text-textPrimary": !editor.isActive("strike"),
          })}
        />
      </CustomIconButton>
      <CustomIconButton
      ...
      >
        <i
          className={classnames("tabler-align-left", {
            "text-textPrimary": !editor.isActive({ textAlign: "left" }),
          })}
        />
      </CustomIconButton>
      <CustomIconButton
...
const MailDetails = (props) => {
  ...
  const handleEmailNavigation = (type) => {
    dispatch(
      navigateEmails({ type, emails, currentEmailId: currentEmail?.id }),
    );
  ...
  const handleMoveAllToSpam = () => {
    dispatch(
      moveEmailsToFolder({ emailIds: [currentEmail?.id], folder: "spam" }),
    );
    setDrawerOpen(false);
  };
  ...
  const handleMoveAllToInbox = () => {
    dispatch(
      moveEmailsToFolder({ emailIds: [currentEmail?.id], folder: "inbox" }),
    );
    setDrawerOpen(false);
  };
        ...
        <>
          <div className="plb-4 pli-6">
            <div className="flex justify-between gap-2">
              ...
              <div className="flex items-center gap-2">
                <IconButton
                  disabled={currentEmail.id === emails[0].id}
                  onClick={() => handleEmailNavigation("prev")}
                >
                  <DirectionalIcon
                    ltrIconClass="tabler-chevron-left"
                    rtlIconClass="tabler-chevron-right"
          ...
          <div className="flex items-center justify-between gap-4 plb-2 pli-6 border-y">
            <div className="flex gap-1">
              <Tooltip
                title={folder === "trash" ? "Delete" : "Move to trash"}
                placement="top"
              >
                <IconButton
                  onClick={(e) => {
              ...
              <OptionMenu
                ...
                options={Object.entries(labelColors).map(([key, value]) => ({
                  ...
                  icon: (
                    <i
                      className={`tabler-circle-filled text-xs text-${value.color}`}
                    />
                  ),
                }))}
              />
            ...
            <div className="plb-5 pli-8 flex flex-col gap-4">
              ...
              {showReplies
                ? currentEmail.replies.map((reply) => (
                    <MailCard key={reply.id} data={reply} isReplies={false} />
                  ))
                : null}
                  ...
                  {!reply ? (
                    <CardContent>
                      <Typography>
                        Click here to
                        <span
                          className="text-primary cursor-pointer mli-1"
                          onClick={() => setReply(true)}
                        >
                          Reply
                        </span>
                        or
                        <span className="text-primary cursor-pointer mis-1">
                          Forward
                        </span>
                      </Typography>
                    </CardContent>
                    ...
                    <div className="flex flex-col gap-y-6">
                      ...
                      <CardActions className="flex items-center justify-end pbs-0">
                        <IconButton>
                          <i
                            className="tabler-trash text-textSecondary"
