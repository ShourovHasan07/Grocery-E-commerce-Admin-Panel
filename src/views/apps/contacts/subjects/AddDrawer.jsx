    ...
    try {
      ...
      if (setType === "edit") {
        response = await pageApiHelper.put(
          `contact-subjects/${data.id}`,
          form,
          token
        );
        toastMessage = "Subject updated successfully";
      } else {
        ...
        toastMessage = "Subject created successfully";
      }
      ...
      if (res?.success && res?.data?.success) {
        if (setType === "edit") {
          const updatedData = userData.map((item) =>
            item.id === res?.data?.contact_subject?.id
              ? { ...item, ...res?.data?.contact_subject }
              : item,
          );

          setData(updatedData);
        } else {
          setData([res?.data?.contact_subject, ...(userData ?? [])]);
        }
    ...
    >
      <div className="flex items-center justify-between plb-5 pli-6">
        <Typography variant="h5">
          {setType == "edit" ? "Edit Subject" : "Add New Subject"}
        </Typography>
