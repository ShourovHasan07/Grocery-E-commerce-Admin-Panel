...
const ListTable = () => {
  ...
  const buildQueryParams = useCallback(() => {
    ...
    // Add status filter (exclude default 'All')
    if (filters.status && filters.status !== "all") {
      params.status = filters.status;
    }

    // Add subject filter (exclude default 'All')
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      if (result.success) {
        const contactsData = result.data?.data?.contacts || [];
  ...
  }, [debouncedSearch, filters.status, filters.subject]);
  ...
  return (
    ...
    >
      <Card>
        ...
        <TableFilters filters={filters} onFiltersChange={handleFiltersChange} token={token} />
