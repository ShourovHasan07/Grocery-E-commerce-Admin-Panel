...
export async function GET(request) {
  ...
  try {
    const result = await routeApiHelper.get('achievements', { pageSize: 200 }, token);

    if (result.success) {
      return NextResponse.json(
        { success: true, data: result.data, message: "achievements fetched successfully" },
        { status: 200 }
      );
    }

    return NextResponse.json(
      { success: false, data: [], message: "achievements not found" },
      { status: 404 }
    );
  } catch (error) {
    return NextResponse.json(
...
export async function POST(request) {
  ...
  try {
    ...
    return NextResponse.json(
      { success: false, data: result.data, message: result.message || "achievements creation failed" },
