...
export async function GET(request) {
  ...
  try {
    ...
    if (result.success) {
      return NextResponse.json(
        { success: true, data: result.data, message: "admins fetched successfully" },
        { status: 200 }
      );
    }
...
export async function POST(request) {
  ...
  try {
    ...
    const result = await routeApiHelper.post('admins', outgoingFormData, token, headerConfig);

    if (result.success) {
      return NextResponse.json(
        { success: true, data: result.data, message: "admins informs submit  successfully" },
        { status: 200 }
      );
    }
